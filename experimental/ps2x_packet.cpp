...
#ifdef ARDUINO
    #define PDEBUGMESSAGE(...)  Serial.printf(__VA_ARGS__)
#else
    #define PDEBUGMESSAGE(...)  printf(__VA_ARGS__)
#endif
...
int main(){
    ...
    ps2x_packet.SetButtonState(PSB_SELECT);
    ps2x_packet.SetButtonState(PSB_R1);
    ps2x_packet.SetAnalogValue(PSS_RX, 132);
    ps2x_packet.SetAnalogValue(PSS_RY, 32);
    ps2x_packet.SetAnalogValue(PSS_LX, 12);
    ps2x_packet.SetAnalogValue(PSS_LY, 222);
    ...
    PDEBUGMESSAGE("SELECT STATE: %d\n", ps2x_packet.GetButtonState(PSB_SELECT));
    PDEBUGMESSAGE("R1 STATE: %d\n", ps2x_packet.GetButtonState(PSB_R1));
    PDEBUGMESSAGE("R2 STATE: %d\n", ps2x_packet.GetButtonState(PSB_R2));
    PDEBUGMESSAGE("LX STATE: %d\n", ps2x_packet.GetAnalogValue(PSS_LX));
    PDEBUGMESSAGE("LY STATE: %d\n", ps2x_packet.GetAnalogValue(PSS_LY));
    PDEBUGMESSAGE("RX STATE: %d\n", ps2x_packet.GetAnalogValue(PSS_RX));
    PDEBUGMESSAGE("RY STATE: %d\n", ps2x_packet.GetAnalogValue(PSS_RY));
